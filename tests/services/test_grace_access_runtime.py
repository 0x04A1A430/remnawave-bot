...
GIB = 1024**3
# The session/panel operations are keyed by the numeric Remnawave id (v3 has no uuid).
PANEL_ID = 9001
...
class FakeRemnawaveApi:
    """Double for the id-keyed panel client."""
    ...
    async def get_user_by_uuid(self, uuid: str, user_id: int | None = None) -> SimpleNamespace | None:
        self.reads.append(user_id)
        return self.user if user_id == self.user.id else None

    async def update_user(self, *, uuid: str, user_id: int | None = None, **kwargs: Any) -> SimpleNamespace:
        self.updates.append({'uuid': uuid, 'user_id': user_id, **kwargs})
...
def make_v2_session_row(*, remnawave_id: int | None = PANEL_ID) -> GraceAccessSessionModel:
    """A row written before the identity backfill: snapshot_version=2 and no id in the JSON blobs.

    The pre-backfill blob format did not carry the panel identity inside
    ``panel_before``/``billing_before``; the reader must fall back to the
    ``remnawave_id`` column of the row.
    """
...
def test_panel_target_serializer_removes_derived_statuses(
    ...
    # The panel user is addressed by its numeric id; the PATCH body carries user_id.
...
async def test_apply_overlay_detaches_external_squad_first_and_addresses_the_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    overlay = make_overlay()
    api = FakeRemnawaveApi(
    ...
    assert api.updates[0] == {'uuid': '', 'user_id': PANEL_ID, 'external_squad_uuid': None}
...
def test_v2_snapshot_row_stays_readable_after_the_identity_backfill() -> None:
    # A v2 row's panel_before blob has no identity key, so the reader falls
    # back to the row's remnawave_id column. Refusing v2 rows would leave such
    # overlays open forever.
    session = _model_to_session(make_v2_session_row())

    assert session.remnawave_id == PANEL_ID
    # panel_before knows no id, so the identity comes from the row column.
    ...
    # blob stored no id, so it reads as None rather than a stale value.
...
def test_saving_a_v2_row_upgrades_it_to_v3_without_erasing_the_identity() -> None:
    ...
    # The identity column IS remnawave_id, so the write keeps it.
