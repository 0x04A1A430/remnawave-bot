...
@pytest.mark.asyncio
async def test_gift_panel_update_delegates_to_subscription_service():
    """The gift panel update is delegated to ``update_remnawave_user``.

    That service already resolves the panel identity and completes an open
    grace session as PAID, so the handler must not talk to the panel itself.
    """
    sub = _sub()
    user = _user()
    db = AsyncMock()

    updated = SimpleNamespace(subscription_url='https://g', happ_crypto_link='gc')
    svc = MagicMock()
    svc.update_remnawave_user = AsyncMock(return_value=updated)
...
@pytest.mark.asyncio
async def test_gift_panel_update_swallows_panel_errors():
    """A panel failure must not break activation — the gift is applied locally."""
    sub = _sub()
    user = _user()
    db = AsyncMock()

    svc = MagicMock()
    svc.update_remnawave_user = AsyncMock(side_effect=RuntimeError('panel down'))

    with patch('app.services.subscription_service.SubscriptionService', return_value=svc):
        await _apply_gift_panel_update(db, sub, user)  # must not raise
