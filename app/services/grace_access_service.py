...
class GraceSessionStore(Protocol):
    """Persistence adapter implemented in the runtime integration."""
...
class GraceAccessService:
    ...
    async def should_suppress_webhook(
        ...
        # expiring the temporary overlay. Billing remains authoritative.
...
def build_panel_overlay(
    ...
    *,
    ...
    # user exactly ``traffic_bytes`` of usable grace traffic.
    ...
    return GracePanelOverlay(
        ...
        # External squads могут давать доступ вне Telegram-сквадов, поэтому
        # на время грейса их отвязываем.
...
def panel_is_safe_pending_source(
    ...
    The sole accepted partial state is an otherwise unchanged original snapshot
    whose external squad has already been detached by the gateway's preflight
    PATCH.
    """
