...
_SNAPSHOT_VERSION = 3
# Version 3 stores the panel identity.  Version 2 rows store the same uuid key
# and remain readable, so a session that predates a panel change stays
# reconcilable.  Refusing v2 here would raise for every such row and their
# overlay would never be rolled back.
...
class RemnawaveGracePanelGateway:
    ...
    async def apply_overlay(self, remnawave_id: int, overlay: GracePanelOverlay) -> None:
        ...
        async with remnawave_service.get_api_client() as api:
            ...
            # client may retry without externalSquadUuid; doing this before
...
class GraceAccessRuntime:
    ...
    async def should_suppress_webhook(
        ...
        *,
        ...
        if self._mode in (GraceAccessMode.DISABLED, GraceAccessMode.OBSERVE):
            # Non-mutating grace: нет оверлеев — нечего подавлять.
            return False
        try:
            if db is not None:
    ...
    async def _recent_candidate_ids(self) -> list[tuple[int, GraceReason]]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # keep the PostgreSQL query plan predictable during a large wave.
...
async def update_panel_user_grace_safe(
    ...
    **update_kwargs: Any,
    ...
    async with grace_sensitive_panel_update(subscription_id) as lease:
        ...
        supplied_uuid = _optional_panel_uuid(update_kwargs.get('uuid'))
        fresh_subscription = lease.subscription
        expected_uuid = _optional_panel_uuid(
            fresh_subscription.remnawave_uuid
            if settings.is_multi_tariff_enabled()
            else (fresh_subscription.user.remnawave_uuid if fresh_subscription.user else None)
        )
        if expected_uuid and supplied_uuid != expected_uuid:
            raise GracePanelError(f'Remnawave panel user changed before subscription {subscription_id} update')
    ...
    *,
    ...
    try:
        ...
        if not already_in_state:
            if enabled:
                raise
            # The disable request may have reached Remnawave despite a timeout.
            # Commit suppression so grace can never re-enable an intentionally
...
def _subscription_to_billing(subscription: Subscription) -> GraceBillingState:
    ...
    return GraceBillingState(
        ...
        # The fork's Tariff model has no ``is_free`` flag; the FREE classification
        # is disabled for the fork.  See the porting report.
        is_free_tariff=bool(tariff and getattr(tariff, 'is_free', False)),
