"""Handler for TabPay balance top-up (tabpay.org)."""
...
TABPAY_PAYMENT_METHODS = {'tabpay', 'tabpay_card', 'tabpay_sbp'}

TABPAY_SERVICE_MAP: dict[str, str | None] = {
    'tabpay': None,
    'tabpay_card': 'card',
    'tabpay_sbp': 'sbp',
}
...
def _extract_service_type(payment_method: str) -> str | None:
    return TABPAY_SERVICE_MAP.get(payment_method)
...
def _display_name_for_method(payment_method: str) -> str:
    if payment_method == 'tabpay_sbp':
        return settings.get_tabpay_sbp_display_name()
    if payment_method == 'tabpay_card':
        return settings.get_tabpay_card_display_name()
    return settings.get_tabpay_display_name()


async def _create_tabpay_payment_and_respond(
    message_or_callback,
    db_user: User,
    db: AsyncSession,
    amount_kopeks: int,
    ...
    """Создаёт платёж TabPay и отправляет покупателю ссылку на payUrl."""
    ...
    logger.info('TabPay payment created', telegram_id=db_user.telegram_id, amount_rub=amount_rub)


@error_handler
async def process_tabpay_payment_amount(
    ...
    """Обрабатывает сумму, введённую пользователем для TabPay."""
    texts = get_texts(db_user.language)
    ...
    min_amount = settings.TABPAY_MIN_AMOUNT_KOPEKS
    max_amount = settings.TABPAY_MAX_AMOUNT_KOPEKS
    ...
    payment_method = data.get('payment_method', 'tabpay')
    payment_method_type = _extract_service_type(payment_method)

    await state.clear()

    await _create_tabpay_payment_and_respond(
...
async def _start_tabpay_topup_impl(
    callback: types.CallbackQuery,
    db_user: User,
    state: FSMContext,
    payment_method: str,
):
    """Стартует FSM ввода суммы для TabPay."""
    texts = get_texts(db_user.language)
    ...
    min_amount = settings.TABPAY_MIN_AMOUNT_KOPEKS // 100
    max_amount = settings.TABPAY_MAX_AMOUNT_KOPEKS // 100
...
@error_handler
async def start_tabpay_topup(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
):
    await _start_tabpay_topup_impl(callback, db_user, state, 'tabpay')


@error_handler
async def start_tabpay_card_topup(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
):
    await _start_tabpay_topup_impl(callback, db_user, state, 'tabpay_card')


@error_handler
async def start_tabpay_sbp_topup(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
    state: FSMContext,
):
    await _start_tabpay_topup_impl(callback, db_user, state, 'tabpay_sbp')
