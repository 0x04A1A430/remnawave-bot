...
PERMANENT_SUBSCRIPTION_YEAR = 2050
...
def format_subscription_end_date(dt: datetime | str | None, format_str: str = '%d.%m.%Y') -> str:
    """Render subscription expiry, treating dates after 2050 as permanent."""
    if not dt:
        return '—'
    if isinstance(dt, str):
        try:
            ...
            return '—'
    if dt.year > PERMANENT_SUBSCRIPTION_YEAR:
        return 'Навсегда'
    return dt.strftime(format_str)
...
def format_subscription_status(is_active: bool, is_trial: bool, end_date: datetime | str, language: str = 'ru') -> str:
    ...
    if end_date > now:
        ...
        if end_date.year > PERMANENT_SUBSCRIPTION_YEAR:
